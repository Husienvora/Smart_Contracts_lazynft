...
describe("NFT721LazyMint", function () {
    ...
    beforeEach(async () => {
        ...
        await deployments.fixture(["LazyNFT"])
    ...
    it("Should be possible to redeem NFT using signed voucher", async () => {
        ...
        expect(
            await nftLazyContract
                .connect(buyer)
                .redeemToken(creator.address, buyer.address, voucher, { value: listingPrice })
        )
        ...
        expect(await ethers.provider.getBalance(creator.address)).to.equal(
            BNtoBigInt(creatorBalanceBefore) + BNtoBigInt(amount)
        )
        expect(await ethers.provider.getBalance(owner.address)).to.equal(
            BNtoBigInt(ownerBalanceBefore) + BNtoBigInt(feeAmount)
        )
        assert.isTrue(
            (await ethers.provider.getBalance(buyer.address)).lt(
                BNtoBigInt(buyerBalanceBefore) - BNtoBigInt(listingPrice)
            )
        )
    })
    ...
    it("Should be possible to redeem NFT using signed voucher several time with single signer", async () => {
        ...
        await nftLazyContract
            .connect(buyer)
            .redeemToken(creator.address, buyer.address, voucher2, { value: listingPrice })
    ...
    it("Should be possible to redeem NFT using signed voucher with different signers", async () => {
        ...
        const voucher2 = await lazyMinter2.createVoucher(
            2,
            listingPrice,
            quantity,
            royaltyBasisPoints,
    ...
    it("Should revert redeem NFT if listingPrice is zero", async () => {
        ...
        await expect(
            nftLazyContract
                .connect(buyer)
                .redeemToken(creator.address, buyer.address, voucher, { value: 0 })
