...
contract MarketNFT721ImplementationV2 is Ownable, ReentrancyGuard {
    ...
    function buyItem(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(_nftContract, _tokenId, _seller, _pricePerItem)
        );
        require(
            ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), _signature) ==
                getSignerAddress(),
            "Invalid signature"
        );
    ...
    function _deduceRoyalties(
        ...
        (address royaltiesReceiver, uint256 royaltiesAmount) = IERC2981Royalties(_nftContract)
    ...
    function acceptOfferWithWETH(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(_nftContract, _tokenId, _offerCreator, _offeredPricePerItem, _WETH)
        );
        require(
            ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), _signature) ==
                getSignerAddress(),
        ...
        require(
            IERC165(_nftContract).supportsInterface(_INTERFACE_ID_ERC721),
            "Invalid NFT address"
        );
        require(
        ...
        require(
            IERC20(_WETH).allowance(_offerCreator, address(this)) == _offeredPricePerItem,
            "Marketplace should be approved as a spender"
        );
        ...
        // Pay royalties if applicable
        if (_checkRoyalties(_nftContract)) {
            saleValue = _deduceRoyaltiesWithWETH(
                _nftContract,
                _tokenId,
                saleValue,
                _offerCreator,
    ...
    /// @return netSaleAmount - the value that will go to the seller after
    ///         deducting royalties
    function _deduceRoyaltiesWithWETH(
        address _nftContract,
        uint256 _tokenId,
        uint256 _grossSaleValue,
        address _offerCreator,
    ...
    function acceptOfferWithERC20(
        ...
        require(
            IERC20(_currency).allowance(_offerCreator, address(this)) == _offeredPricePerItem,
            "Marketplace should be approved as a spender"
        );
        ...
        // Pay royalties if applicable
        if (_checkRoyalties(_nftContract)) {
            saleValue = _deduceRoyaltiesWithERC20(
                _nftContract,
                _tokenId,
                saleValue,
                _offerCreator,
    ...
    /// @return netSaleAmount - the value that will go to the seller after
    ///         deducting royalties
    function _deduceRoyaltiesWithERC20(
        address _nftContract,
        uint256 _tokenId,
        uint256 _grossSaleValue,
        address _offerCreator,
